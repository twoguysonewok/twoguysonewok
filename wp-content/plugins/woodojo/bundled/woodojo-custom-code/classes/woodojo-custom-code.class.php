...
Description: Add custom CSS code or HTML in the <head> or before the closing </body> tag.
...
class WooDojo_CustomCode {
	...
	public function enqueue_custom_css () {
		wp_register_style( $this->token, site_url( '/?' . $this->token . '=css' ), '', '1.0.0', 'screen' );
	...
	public function output_custom_html () {
		switch ( current_filter() ) {
			case 'wp_head':
				echo $this->settings['custom-html-code-head'];
			break;

			case 'wp_footer':
				echo $this->settings['custom-html-code-footer'];
			break;
		}
