...
class WooThemes_Transmitter {
				...
				foreach ( $messages as $k => $v ) {
					if ( isset( $v->messages ) && count( $v->messages ) > 0 ) {
						...
						foreach ( $v->messages as $i => $j ) {
							...
							$mark_as_read = wp_nonce_url( admin_url( 'admin-ajax.php?action=' . $this->token . '-mark-as-read&message_id=' . $j->message_id ), $this->token . '-mark-as-read' );
							...
							$wp_admin_bar->add_menu( array( 'parent' => $this->token . '-messages', 'id' => 'message-id-' . ( $j->message_id ), 'title' => '<a href="' . esc_url( $mark_as_read ) . '" class="mark-as-read" title="' . esc_attr__( 'Mark as Read', 'wootransmitter' ) . '">' . __( 'Mark as Read', 'wootransmitter' ) . '</a><span class="message-contents"><span class="title">' . $j->title . '</span><span class="excerpt">' . $excerpt . '</span><span class="application ' . sanitize_title_with_dashes( $v->application->name ) . '">' . $v->application->name . '</span></span>' ) );

							$wp_admin_bar->add_menu( array( 'parent' => 'message-id-' . ( $j->message_id ), 'id' => 'message-id-' . ( $j->message_id ) . '-full', 'title' => '<span class="message-contents"><span class="excerpt">' . nl2br( $j->message ) . '</span></span>' ) );
						}
					}
				}
			} else {
	...
	public function get_messages ( $status = 'all' ) {
		...
		if ( $status == 'unread' ) {
			...
			foreach ( $data as $k => $v ) {
				foreach ( $v->messages as $i => $j ) {
					if ( in_array( $j->message_id, $read_messages ) ) {
	...
	private function get_message_ids ( $status = 'all' ) {
		...
		if ( is_array( $data ) && count( $data ) > 0 ) {
			foreach ( $data as $k => $v ) {
				if ( isset( $v->messages ) && count( $v->messages ) > 0 ) {
					foreach ( $v->messages as $i => $j ) {
						if ( isset( $j->message_id ) && ( $j->message_id != '' ) ) {
							$ids[] = $j->message_id;
						}
					}
				}
